...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, eosio::chain::asset::from_string("999.0000 " CORE_SYMBOL_NAME) );
   }
   produce_blocks(1);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, eosio::chain::asset::from_string("10000.0000 " CORE_SYMBOL_NAME) );
   }
   produce_blocks(1);
