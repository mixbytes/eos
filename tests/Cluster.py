...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodeArgs=None, useBiosBootFile=True, specificExtraNodeArgs=None, onlySetProds=False,
        ...
        extraNodeArgs: string of arguments to pass through to each nodoes instance (via --daobet-node flag on launcher)
        ...
        specificExtraNodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-daobet-node flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        if specificExtraNodeArgs is not None:
            ...
            for nodeNum,arg in specificExtraNodeArgs.items():
                ...
                cmdArr.append("--specific-daobet-node")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-daobet-node")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.ServerName, startedNodes, len(nodes)))
            return False
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        transferAmountStr=Node.currencyIntToStr(transferAmount, Utils.CoreSym)
        node=self.nodes[0]
        fromm=source
        to=accounts[0]
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n BET and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        ...
        initialFunds="1000000.0000 {0}".format(Utils.CoreSym)
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        """Create 'prodCount' init accounts and deposits 10000000000 BET in each. If prodCount is -1 will initialize all possible producers.
        ...
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (eosioAccount.name, Utils.CoreSym)
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (eosioAccount.name, Utils.CoreSym)
        ...
        expectedAmount="1000000000.0000 {0}".format(Utils.CoreSym)
        ...
        if not biosNode.waitForTransInBlock(transId):
            ...
            return None
        action="init"
        data="{\"version\":0,\"core\":\"4,%s\"}" % (Utils.CoreSym)
        opts="--permission %s@active" % (eosioAccount.name)
    ...
    def pgrepEosServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.ServerName)
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster node instances. allInstances will kill all node instances running on the system."""
