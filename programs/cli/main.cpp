...
/**
  ...
  @section intro Introduction to daobet-cli

  `daobet-cli` is a command line tool that interfaces with the REST api exposed by @ref daobet-node. In order to use `daobet-cli` you will need to
  have a local copy of `daobet-node` running and configured to load the 'eosio::chain_api_plugin'.

   daobet-cli contains documentation for all of its commands. For a list of all commands known to daobet-cli, simply run it with no arguments:
```
$ ./daobet-cli
Command Line Interface to EOSIO Client
Usage: daobet-cli [OPTIONS] SUBCOMMAND
...
Usage: ./daobet-cli create SUBCOMMAND

Subcommands:
  key                         Create a new keypair and print the public and private keys
  account                     Create a new account on the blockchain (assumes system contract does not restrict RAM usage)

$ ./daobet-cli create account
Create a new account on the blockchain (assumes system contract does not restrict RAM usage)
Usage: ./daobet-cli create account [OPTIONS] creator name OwnerKey ActiveKey
...
chain::action create_delegate(
      const name& from,
      const name& receiver,
      const asset& net,
      const asset& cpu,
      const asset& vote, // DAOBET
      bool transfer) {
   fc::variant act_payload = fc::mutable_variant_object()
         ("from", from.to_string())
         ...
         ("stake_vote_quantity", vote.to_string()) // DAOBET
         ("transfer", transfer);
...
void ensure_wallet_running(CLI::App* app) {
    ...
    // This extra check is necessary when running daobet-cli like this: ./daobet-cli ...
    if (binPath.filename_is_dot())
        binPath.remove_filename();
    binPath.append(key_store_executable_name); // if daobet-cli and daobet-wallet are in the same installation directory
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         ...
         createAccount->add_option("--stake-vote", stake_vote,
                                   (localized("The amount of tokens delegated for CPU bandwidth")))->required(); // DAOBET
      ...
      createAccount->set_callback([this] {
            ...
            if (!simple) {
               ...
               auto vote = to_asset(stake_vote); // DAOBET
               if ( net.get_amount() != 0 || cpu.get_amount() != 0 ) {
                  action delegate = create_delegate( creator, account_name, net, cpu, vote, transfer); // DAOBET
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so daobet-cli can still work with old buggy daobet-node versions
                               // Change to voter.value when daobet-cli no longer needs to support daobet-node versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when daobet-cli no longer needs to support daobet-node versions older than 1.5.0
...
struct delegate_bandwidth_subcommand {
   ...
   string stake_vote_amount; // DAOBET
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_vote_quantity", stake_vote_amount, localized("The amount of tokens to stake for vote bandwidth"))->required(); // DAOBET
      ...
      delegate_bandwidth->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("stake_vote_quantity", to_asset(stake_vote_amount)) // DAOBET
                  ("transfer", transfer);
...
struct undelegate_bandwidth_subcommand {
   ...
   string unstake_vote_amount; // DAOBET
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_vote_quantity", unstake_vote_amount, localized("The amount of tokens to undelegate for vote bandwidth"))->required(); // DAOBET
      ...
      undelegate_bandwidth->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("unstake_vote_quantity", to_asset(unstake_vote_amount)); // DAOBET
         auto accountPermissions = get_account_permissions(tx_permission, {from_str,config::active_name});
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so daobet-cli can still work with old buggy daobet-node versions
                               // Change to newname.value when daobet-cli no longer needs to support daobet-node versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
   ...
   if (!json_format) {
      ...
      if ( res.total_resources.is_object() ) {
         ...
         if( net_total.get_symbol() != unstaking.get_symbol() ) {
            // Core symbol of daobet-node responding to the request is different than core symbol built into daobet-cli
      ...
      if( res.refund_request.is_object() ) {
         ...
         ///@{
         /// HAYA: [cyb-445] refund time 14 days
         ...
         asset vote = asset::from_string( obj["vote_amount"].as_string() ); // DAOBET
...
int main( int argc, char** argv ) {
   ...
   app.set_callback([&app]{ ensure_wallet_running(&app);});
...
//                                ->check(CLI::ExistingFile);
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so daobet-cli can still work with old buggy daobet-node versions
                                 // Change to name(proposal_name).value when daobet-cli no longer needs to support daobet-node versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when daobet-cli no longer needs to support daobet-node versions older than 1.5.0
