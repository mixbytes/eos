      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/haya")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/haya")
            fi
...
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/haya_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
