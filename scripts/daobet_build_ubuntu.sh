...
if [ -z $SKIP_DEPS_CHECK ]; then
    ...
    if [ ! -d $MONGO_C_DRIVER_ROOT ]; then
        ...
        && mkdir -p cmake-build \
        && cd cmake-build \
        && $CMAKE -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PREFIX -DENABLE_BSON=ON -DENABLE_SSL=OPENSSL -DENABLE_AUTOMATIC_INIT_AND_CLEANUP=OFF -DENABLE_STATIC=ON .. \
        && make -j"${JOBS}" \
        && make install \
        && cd ../.. \
    ...
    if [ ! -d $MONGO_CXX_DRIVER_ROOT ]; then
        ...
        && $CMAKE -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$INSTALL_PREFIX .. \
